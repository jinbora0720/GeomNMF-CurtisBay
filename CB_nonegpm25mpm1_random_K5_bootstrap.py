...
# reference H 
results = load(outdir/"sourceXray_CB_nonegpm25mpm1_random_K5.joblib")
H_star = results[30*K]["H_star"][[1,0,2,3,4],:]
...
H_star_hat, W_tilde_hat, mu_tilde_hat, C_hat, logvol_hat = sourceXray(Yb, K, candidate_method="random", 
                                                                      T=20000, topk=1, max_K=40*K, seed=seed_rep, prune=True, min_K=30*K, verbose=True)[0]
end = time.time()
