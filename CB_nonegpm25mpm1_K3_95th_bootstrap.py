...
# standardization using 95th percentile
Y = Y_org.copy()
for col in Y.columns:
    iqnr_j = np.diff(np.quantile(Y[col], [0, 0.95], axis=0))
    min_j = Y[col].min()
    if min_j < 0: 
        Y[col] = Y[col] - min_j
    Y[col] = Y[col]/iqnr_j
...
dump(results_boots, outdir/"diffperc"/f"CB_nonegpm25mpm1_K3_95th_bootstrap_rep{rep}.joblib")   
