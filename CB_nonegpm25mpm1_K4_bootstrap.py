...
# reference H 
results = load(outdir/"sourceXray_CB_nonegpm25mpm1_K4.joblib")
H_star = results[50*K]["H_star"][[0,2,3,1],:]
...
# sourceXray
start = time.time()
H_star_hat, W_tilde_hat, mu_tilde_hat, C_hat, logvol_hat = sourceXray(Yb, K, seed=seed_rep, prune=True, min_K=50*K, verbose=False)[0]
end = time.time()
...
dump(results_boots, outdir/f"CB_nonegpm25mpm1_K4_bootstrap_rep{rep}.joblib")   
